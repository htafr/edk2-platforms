...
#include <LitexRocketCore.h>
#include <Library/LitexRocket.h>
  ...
  )
{
  ...
  BOOLEAN MCSupport = FALSE;

  DEBUG ((DEBUG_INFO, "Building Rocket Core processor information HOB\n"));

  HartIdNumber = 0;
  ParentCoreGuid = PcdGetPtr(PcdLitexRocketCoreGuid);
  for (; HartIdNumber < (FixedPcdGet32 (PcdNumberofRocketCores) + (UINT32)MCSupport); HartIdNumber ++) {
    ...
    if (EFI_ERROR (Status) && Status != EFI_UNSUPPORTED) {
      DEBUG ((DEBUG_ERROR, "Failed to build Rocket processor information HOB\n"));
      ASSERT (FALSE);
    }  else {
      if (!EFI_ERROR (Status)) {
        DEBUG ((DEBUG_INFO, "Support Rocket application core on Litex Rocket platform, HOB Data at address 0x%x\n", GuidHobData));
      }
    }
  }
  DEBUG ((DEBUG_INFO, "Support %d Rocket application cores on Litex Rocket platform\n", HartIdNumber - (UINT32)MCSupport));
  ...
  )
{
  ...
  L2CacheDataHob.ProcessorGuid = *((EFI_GUID *)PcdGetPtr (PcdLitexRocketCoreGuid));
  ...
  L2CacheDataHob.SmbiosType7Cache.MaximumCacheSize = 0x4000;
  L2CacheDataHob.SmbiosType7Cache.InstalledSize = 0x4000;
  ...
  L2CacheDataHob.SmbiosType7Cache.ErrorCorrectionType = CacheErrorUnknown;
  L2CacheDataHob.SmbiosType7Cache.SystemCacheType = CacheTypeUnified;
  L2CacheDataHob.SmbiosType7Cache.Associativity = CacheAssociativity4Way;
...
)
{
  ...
  DEBUG ((DEBUG_INFO, "Rocket Core SMBIOS DATA HOB at address 0x%x\n", SmbiosHobPtr));
