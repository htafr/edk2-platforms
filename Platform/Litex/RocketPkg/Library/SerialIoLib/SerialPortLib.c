...
/**  
 * For Litex I have to change the registers and the GetChar and PutChar functions
 **/
...
#define SYS_CLK        FixedPcdGet32(PcdRocketPlatformSystemClock)
...
{
  STATIC volatile UINT32 * const uart = (UINT32 *)FixedPcdGet32(PcdRocketUartBase);
...
{
  while (GetReg (UART_REG_TXFULL));

  SetReg (UART_REG_RXTX, Ch);
}
...
UINT32 SifiveUartGetChar (VOID)
{
  ...
  if (GetReg (UART_REG_RXEMPTY)) {
    return -1;
  }

  Ret = GetReg (UART_REG_RXTX);
  SetReg (UART_REG_EV_PENDING, UART_EV_RX);
  ...
  )
{
  ...
  litex_uart_init (FixedPcdGet32 (PcdRocketUartBase));
